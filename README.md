...
| 창 | 역할 | 여는 법 |
| --- | --- | --- |
| **오브** | 작업 표시줄 위 바닥을 걸어 다니는 펫(피코·모푸·새싹·노바·모치 중 하나, 설정에서 고름). 남은 할 일·오늘 일정 개수 배지. 알림이 오면 옆에 말풍선과 `!!` 표정, 회의 중이면 걸음을 멈추고 생각하는 표정, 자리를 비우면 잠들고 돌아오면 손을 흔듦. 서 있을 때는 마우스 쪽을 봄 | 앱을 켜면 늘 떠 있음. 끌어서 옮기면 놓은 자리 바닥으로 떨어지고, 빠르게 던지면 벽·바닥에 튕긴 뒤 어지러워함. 안 보이면 트레이의 "오브 표시/숨김" 또는 설정의 "오브 위치 초기화" |
| **패널** | 펫 옆에 뜨는 작은 창. 탭: 할 일(추가·완료·알림), 오늘(일정·마감), 실행(런처와 `/` 명령), 클립보드, 펫(캐릭터·크기·돌아다니기). 머리의 "Orbit 열기"로 큰 창 | 펫 클릭. 다른 곳을 클릭하거나 <kbd>Esc</kbd>를 누르면 사라짐. 펫 오른쪽에 붙고, 자리가 없으면 왼쪽 |
...
└─ windows/
   ...
   ├─ orb/                      오브 (Pet.tsx 펫+배지+클릭/잡기, petLoop.ts 배회·커서 바라보기·낙하·던지기, motion.ts 순수 물리, petStore, pet/ 캐릭터 SVG 5종·표정·pet.css 골격 애니메이션, bubbleStore·Bubble 알림 말풍선 큐, status 자리 비움·회의)
   ├─ panel/                    펫 패널 (PanelApp.tsx 탭 5개 — 모듈의 TodoPanel·Agenda·Launcher·ClipboardPanel·PetSettings 재사용)
