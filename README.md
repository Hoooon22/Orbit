...
| 창 | 역할 | 여는 법 |
| --- | --- | --- |
| **오브** | 작업 표시줄 위 바닥을 걸어 다니는 펫(피코·모푸·새싹·노바·모치 중 하나, 설정에서 고름). 마우스를 올리면 남은 할 일·오늘 일정 개수가 툴팁으로. 알림이 오면 옆에 말풍선과 `!!` 표정, 회의 중이면 걸음을 멈추고 생각하는 표정, 자리를 비우면 잠들고 돌아오면 손을 흔듦. 서 있을 때는 마우스 쪽을 봄 | 앱을 켜면 늘 떠 있음. 끌어서 옮기면 놓은 자리 바닥으로 떨어지고, 빠르게 던지면 벽·바닥에 튕긴 뒤 어지러워함. 안 보이면 트레이의 "오브 표시/숨김" 또는 설정의 "오브 위치 초기화" |
