...
화면 바닥을 걸어 다니는 작은 캐릭터(오브) 하나로 부리는 Windows 개인 비서. 오브를 누르면 옆에 작은 패널(할 일·오늘·실행·클립보드·펫)이 뜨고, 거기서 Orbit 창을 열면 메모·할 일·일정·클립보드·앱 실행에 더해 메모 폴더에서 도는 AI 터미널(Claude Code)까지 한 화면에서 다룹니다. (이전 이름: DesktopMemo)
...
| 창 | 역할 | 여는 법 |
| --- | --- | --- |
| **오브** | 작업 표시줄 위 바닥을 걸어 다니는 펫(피코·모푸·새싹·노바·모치 중 하나, 설정에서 고름). 마우스를 올리면 남은 할 일·오늘 일정 개수가 툴팁으로. 알림이 오면 옆에 말풍선과 `!!` 표정, 회의 중이면 걸음을 멈추고 생각하는 표정, 자리를 비우면 잠들고 돌아오면 손을 흔듦 | 앱을 켜면 늘 떠 있음. 끌어서 옮기면 놓은 자리 바닥으로 떨어지고, 빠르게 던지면 벽·바닥에 튕긴 뒤 어지러워함. 피코는 세게 부딪히면 머리·몸통·팔다리 조각이 각자 화면을 굴러다니며 흩어졌다가, 다 멈추면 머리가 일어나고 조각이 차례로 붙어 조립됨. 안 보이면 트레이의 "오브 표시/숨김" 또는 설정의 "오브 위치 초기화" |
| **패널** | 펫 옆에 뜨는 작은 창. 탭: 빠른 메모, 할 일(추가·완료·알림), 장기(기억해야 할 일), AI(Orbit 창과 같은 터미널 세션), 펫(캐릭터·크기·돌아다니기·투명도). 머리의 "Orbit 열기"로 큰 창 | 펫 클릭. 다른 곳을 클릭하거나 <kbd>Esc</kbd>를 누르면 사라짐. 펫 오른쪽에 붙고, 자리가 없으면 왼쪽 |
| **Orbit** | 홈(오늘·할 일·실행·클립보드), AI, 메모, 할 일, 캘린더, 클립보드, 런처, 통계, 설정 | 패널의 "Orbit 열기", 트레이 아이콘 클릭, 작업 표시줄의 Orbit, <kbd>Alt</kbd>+<kbd>Space</kbd>, <kbd>Ctrl</kbd>+<kbd>Alt</kbd>+<kbd>M</kbd> |

영역 캡처·색상 추출 중에는 모니터를 덮는 네 번째 창(오버레이)이 잠깐 뜨고 끝나면 사라집니다.

트레이 아이콘을 우클릭하면: **Orbit 열기 · 오브 표시/숨김 · ☐ 회의 모드 · ☐ 알림 일시중지 · 영역 캡처 · 색상 추출 · 데이터 폴더 열기 · 업데이트 확인 · 종료**. "알림 일시중지"는 켜 둔 동안 할 일 알림을 미루며(끄면 다음 분에 밀린 알림이 옴), 앱을 다시 켜면 풀립니다.
...
가운데 칸 맨 아래에는 접힌 **빠른 메모**가 있습니다. 토글 줄을 누르면 펼쳐져 홈에서 바로 적을 수 있고, 펼침·접힘 상태는 설정에 남아 다음 실행에도 그대로입니다.

### AI

왼쪽 레일의 **AI**를 누르면 Orbit 창 안에 터미널이 열리고 [Claude Code](https://claude.com/claude-code)가 바로 실행됩니다. 작업 폴더는 메모가 있는 `문서\Orbit`이라, 메모를 읽거나 고쳐 달라고 그대로 시킬 수 있습니다.
...
- 편집기는 노션처럼 서식이 바로 보이지만 저장되는 파일은 마크다운 그대로입니다. `# `, `- `, `1. `, `> `, `---`로 제목·목록·인용문·구분선, 마크다운 표, 클립보드 이미지 붙여넣기(`.assets`에 저장), 헤더의 **캡처**로 화면 일부를 끌어 커서 자리에 넣기(단축키 캡처는 클립보드에만 가고 메모에는 안 들어감), 링크 <kbd>Ctrl</kbd>+클릭, 입력이 멈추면 0.5초 뒤 자동 저장(<kbd>Ctrl</kbd>+<kbd>S</kbd> 즉시), <kbd>Ctrl</kbd>+마우스휠로 글자 크기
- 메모마다 마지막으로 보던 자리(커서·스크롤)를 기억해, 다시 열면 그 자리에서 이어 봅니다. 이 기기에만 남는 값이라 메모 파일에는 들어가지 않습니다
- 빠른 메모는 아무 때나 적어 두는 임시 공간입니다. **폴더로 저장**은 새 메모로 옮기고, **이어서 붙이기**는 있는 메모 끝에 구분선과 오늘 날짜를 넣고 덧붙입니다. 본문 일부를 끌어 선택한 뒤 누르면 그 부분만 옮겨집니다

### 할 일과 리마인더

할 일은 두 묶음입니다. **⚡ 당장 할 일**은 지금 신경 써야 하는 것이고, **🗂 기억해야 할 일**은 급하지 않지만 잊으면 안 되는 것(장기)입니다. 홈과 패널의 개수는 당장 할 일만 셉니다. 추가할 때 입력창 앞의 ⚡/🗂 버튼으로 묶음을 고르고, 할 일 화면에서 "장기로 →"/"← 당장으로"로 옮깁니다. 홈에서는 기억해야 할 일이 접힌 채로 아래에 있습니다.
...
- **알림** — 할 일 화면에서 항목마다 마감 시각과 "정각 / 10분 전 / 1시간 전 / 하루 전"을 고릅니다. 시각이 되면 **오브 옆에 말풍선**이 떠서 6~15초 머물고(클릭하면 할 일 화면), 오브를 숨겨 둔 상태면 대신 Windows 알림이 뜹니다. 앱이 꺼져 있던 동안 지난 알림은 다음 실행 때 "놓친 알림"으로 한 번에 묶여 옵니다.
- 말풍선은 오브 왼쪽으로 펼쳐지고, 오브가 화면 왼쪽 끝에 있으면 오른쪽으로 펼쳐집니다. 알림이 연달아 오면 최대 5개까지 차례로 보여 줍니다. 펫을 잡고 있는 동안은 미뤄 뒀다가 놓으면 띄웁니다.
...
- **회의 모드**(설정 또는 트레이에서 켬): 시각이 있는 일정이 진행 중이면 그동안 할 일 알림을 미루고, 펫이 걸음을 멈추고 생각하는 표정을 합니다(마우스를 올리면 끝 시각이 보임). 시작할 때 말풍선으로 한 번 알립니다. 종일 일정과 23시간 이상 일정은 회의로 보지 않고, 숨긴 구글 일정 제목도 무시합니다. 회의가 끝나면 다음 분에 미룬 알림이 옵니다(5분 넘게 밀린 건 "놓친 알림"으로 묶임)
...
<kbd>Alt</kbd>+<kbd>Space</kbd>(설정에서 변경 가능)를 누르면 Orbit 창이 최대화된 채 열리고 실행 칸에 커서가 갑니다. 앱 이름을 치고 <kbd>Enter</kbd>. 창이 이미 떠 있거나 최소화돼 있을 때는 보던 화면 그대로 최대화해 앞으로 가져옵니다.
...
Windows 로그인 시 자동 시작, 전역 단축키(빠른 메모·런처·영역 캡처·색상 추출), 테마, Orbit 창 항상 위에 고정, 메모 글자 크기, 오브(펫 캐릭터·크기(작게/보통/크게)·돌아다니기 빈도·투명도 0~90%·표시 — 펫 패널의 펫 탭에서도 같은 항목을 바꿀 수 있음), 회의 모드, 구글 캘린더, 클립보드 기록, 사용 통계 기록, 전체 백업. 설정은 메모 폴더의 `.settings.json`에 저장되어 앱을 다시 설치해도 남습니다.

**전체 백업**은 메모 폴더 전체(메모·이미지·할 일·일정·설정)를 `Orbit-2026-09-15-143012.zip`처럼 날짜가 붙은 zip 하나로 묶습니다. 기본 위치는 `문서\Orbit 백업`이고 **폴더 변경**으로 바꿀 수 있습니다(메모 폴더 안은 안 됨). 설정 화면에 최근 백업 시각이 보입니다. 되돌리려면 zip을 풀어 `문서\Orbit`에 덮어쓰면 됩니다. `%LOCALAPPDATA%`의 기기별 데이터(구글 토큰·클립보드 기록 등)는 백업에 넣지 않습니다.
...
```
dashboard.html / orb.html / panel.html / capture.html   창마다 HTML 엔트리 하나 (Orbit 창 / 오브 / 펫 패널 / 캡처 오버레이)
src/
...
└─ windows/
   ...
   ├─ orb/                      오브 (Pet.tsx 펫+클릭/잡기, petLoop.ts 배회·낙하·던지기·산산조각, motion.ts 순수 물리, petStore, pet/ 캐릭터 SVG 5종·표정·pet.css 골격 애니메이션, bubbleStore·Bubble 알림 말풍선 큐, status 자리 비움·회의)
   ├─ panel/                    펫 패널 (PanelApp.tsx 탭 5개 — 모듈의 Editor(compact)·TodoPanel(only)·TerminalView·PetSettings 재사용)
   └─ capture/                  캡처 오버레이 (드래그 선택, 12배 루페)
src-tauri/src/
├─ lib.rs                       트레이(체크 항목은 set_checked로 갱신), 전역 단축키(설정 기반), 창 이벤트, 파일 워처, 자동 시작
├─ meeting.rs                   진행 중인 일정 판정(로컬+구글, 종일·23h+ 제외) → 알림 게이트·오브 표시·트레이 체크
├─ orb.rs                       오브 창 크기(펫 크기별)·바닥 배치(SetWindowPos·캡션 스타일 제거), 말풍선용 창 넓히기·되돌리기, 드래그 폴링(커서·버튼), Orbit 창 열기·토글
├─ panel.rs                     펫 패널 창: 펫 옆 자리 계산(오른쪽, 없으면 왼쪽), 토글, 포커스 잃으면 숨김(직후 재열기 방지 300ms)
├─ reminders.rs                 1분마다 할 일을 읽어 알림(오브 보이면 말풍선 이벤트, 숨김이면 Windows 토스트), 회의·일시중지 중엔 건너뜀, 발송 장부, 놓친 알림 요약
...
오브·Orbit 창은 투명·프레임 없는 창이라 `shadow: false`가 필수(아니면 흰 테두리가 생김)이고, 오브는 tao가 붙이는 `WS_CAPTION` 스타일을 떼어내야 캐릭터 크기(폭 68~125px)로 작아집니다(`orb.rs`). 오브는 window-state 플러그인에서 제외해 x만 설정 파일에 둡니다 — y는 늘 그 모니터 작업 영역의 바닥이고, 걷기·낙하·던지기는 프론트(`petLoop.ts`)가 `setPosition`으로 옮기며 위치 저장은 사용자가 끌어 놓았을 때만 합니다(`onMoved`는 프로그램 이동에도 오기 때문). `startDragging()`은 놓는 순간을 알려 주지 않아 드래그 중 16ms마다 마우스 버튼과 커서를 폴링해 놓는 순간과 던진 속도를 잽니다. 말풍선은 별도 창이 아니라 오브 창을 `SetWindowPos`로 232px 넓혀 그리는 것이라, 넓어진 동안은 잡을 수 없고 잡기 전에 먼저 접습니다. 캐릭터 그림과 표정·동작(pet/)은 WorkPet에서 가져왔습니다.
