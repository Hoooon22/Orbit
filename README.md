...
화면 구석에 늘 떠 있는 작은 구슬(오브) 하나로 부리는 Windows 개인 비서. 오브를 누르면 Orbit 창이 열리고, 거기서 메모·할 일·일정·클립보드·앱 실행에 더해 메모 폴더에서 도는 AI 터미널(Claude Code)까지 한 화면에서 다룹니다. (이전 이름: DesktopMemo)
...
| 창 | 역할 | 여는 법 |
| --- | --- | --- |
| **오브** | 화면 구석의 구슬. 시각, 남은 할 일 개수, 오늘 일정 개수. 알림이 오면 옆에 말풍선. 회의 중이면 시각 대신 끝 시각, 자리를 비우면 흐려지고, 닫지 않은 알림이 있으면 위성이 점멸 | 앱을 켜면 늘 떠 있음. 끌어서 옮김. 안 보이면 트레이의 "오브 표시/숨김" 또는 설정의 "오브 위치 초기화" |
...
- 말풍선은 오브 왼쪽으로 펼쳐지고, 오브가 화면 왼쪽 끝에 있으면 오른쪽으로 펼쳐집니다. 알림이 연달아 오면 최대 5개까지 차례로 보여 줍니다.
...
- **회의 모드**(설정 또는 트레이에서 켬): 시각이 있는 일정이 진행 중이면 그동안 할 일 알림을 미루고, 오브의 시계가 `~끝 시각`으로 바뀌며 원판 테두리가 강조색이 됩니다. 시작할 때 말풍선으로 한 번 알립니다. 종일 일정과 23시간 이상 일정은 회의로 보지 않고, 숨긴 구글 일정 제목도 무시합니다. 회의가 끝나면 다음 분에 미룬 알림이 옵니다(5분 넘게 밀린 건 "놓친 알림"으로 묶임)
...
Windows 로그인 시 자동 시작, 전역 단축키(빠른 메모·런처·영역 캡처·색상 추출), 테마, Orbit 창 항상 위에 고정, 메모 글자 크기, 오브 표시와 투명도, 회의 모드, 구글 캘린더, 클립보드 기록, 사용 통계 기록. 설정은 메모 폴더의 `.settings.json`에 저장되어 앱을 다시 설치해도 남습니다.
...
└─ windows/
   ...
   ├─ orb/                      오브 (Orb.tsx 구슬, OrbApp.tsx 위치 저장·투명도, bubbleStore·Bubble 알림 말풍선 큐, status 자리 비움·회의)
...
├─ orb.rs                       오브 창 배치(SetWindowPos·캡션 스타일 제거), 말풍선용 창 넓히기·되돌리기, Orbit 창 열기·토글
...
오브·Orbit 창은 투명·프레임 없는 창이라 `shadow: false`가 필수(아니면 흰 테두리가 생김)이고, 오브는 tao가 붙이는 `WS_CAPTION` 스타일을 떼어내야 72px로 작아집니다(`orb.rs`). 오브는 window-state 플러그인에서 제외해 위치를 설정 파일에 따로 둡니다. 말풍선은 별도 창이 아니라 오브 창을 `SetWindowPos`로 232px 넓혀 그리는 것이라, 넓어진 동안의 창 이동은 위치로 저장하지 않습니다.
