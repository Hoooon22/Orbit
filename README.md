...
| 창 | 역할 | 여는 법 |
| --- | --- | --- |
| **오브** | 작업 표시줄 위 바닥을 걸어 다니는 펫(피코·모푸·새싹·노바·모치 중 하나, 설정에서 고름). 마우스를 올리면 남은 할 일·오늘 일정 개수가 툴팁으로. 알림이 오면 옆에 말풍선과 `!!` 표정, 회의 중이면 걸음을 멈추고 생각하는 표정, 자리를 비우면 잠들고 돌아오면 손을 흔듦. 서 있을 때는 마우스 쪽을 봄 | 앱을 켜면 늘 떠 있음. 끌어서 옮기면 놓은 자리 바닥으로 떨어지고, 빠르게 던지면 벽·바닥에 튕긴 뒤 어지러워함. 피코는 세게 부딪히면 머리·몸통·팔다리 조각이 각자 화면을 굴러다니며 흩어졌다가, 다 멈추면 머리가 일어나고 조각이 차례로 붙어 조립됨. 안 보이면 트레이의 "오브 표시/숨김" 또는 설정의 "오브 위치 초기화" |
...
Windows 로그인 시 자동 시작, 전역 단축키(빠른 메모·런처·영역 캡처·색상 추출), 테마, Orbit 창 항상 위에 고정, 메모 글자 크기, 오브(펫 캐릭터·크기(작게/보통/크게)·돌아다니기 빈도·투명도 0~90%·표시 — 펫 패널의 펫 탭에서도 같은 항목을 바꿀 수 있음), 회의 모드, 구글 캘린더, 클립보드 기록, 사용 통계 기록. 설정은 메모 폴더의 `.settings.json`에 저장되어 앱을 다시 설치해도 남습니다.
...
└─ windows/
   ...
   ├─ orb/                      오브 (Pet.tsx 펫+클릭/잡기, petLoop.ts 배회·커서 바라보기·낙하·던지기, motion.ts 순수 물리, petStore, pet/ 캐릭터 SVG 5종·표정·pet.css 골격 애니메이션, bubbleStore·Bubble 알림 말풍선 큐, status 자리 비움·회의)
